...
client.on("message", async message => {
    ...
    if (!message.content.startsWith(prefix) && message.content.includes(client.user.id))
        if (!message.guild.me.permissionsIn(message.channel).has("EMBED_LINKS"))
            message.reply(new Discord.MessageEmbed().setColor(config.colors.yes).setAuthor(`${message.author.username}, My prefix is ${prefix}`, message.author.displayAvatarURL({
                dynamic: true
            }), "https://mitsuya"));
        else
            message.reply(`${message.author.username}, My prefix is ${prefix}`)
    ...
    if (command) {
        ...
        return message.reply(`Unkown command, try: ${prefix}play`)
});
